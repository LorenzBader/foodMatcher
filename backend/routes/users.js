...
db.connect().then(() => {
    console.log("Users Route: Connected to DB");
}).catch(err => {
    console.error("Users Route: DB Connection Error:", err);
});
...
router.get('/login', async function(req, res, next) {
    console.log("Login request received:", req.query); // Log incoming data
    const { mail, pw } = req.query

    if (!mail || !pw) {
        console.log("Login failed: Missing mail or pw");
...
router.get('/register', async function(req, res, next) {
    console.log("Register request received:", req.query); // Log incoming data
    const { name, mail, pw } = req.query

    // Validate input
    if (!name || !mail || !pw) {
        console.log("Registration failed: Missing input. Received:", {name, mail, pw});
        return res.status(400).send({ status: 'fail', message: 'Missing name, mail, or pw' });
    }
